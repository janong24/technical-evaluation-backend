...
describe("FileStorage", () => {
  ...
  it("should allow uploading a file and list it afterward", async ({
    expect,
  }) => {
  ...
  it("should download a previously uploaded file and match its content", async ({
    expect,
  }) => {
    const fileName = "testFile.txt";
  ...
  it("should list all uploaded files", async ({ expect }) => {
    ...
    for (const fileName of files) {
      const fileReadable = await stringToReadableStream(
  ...
  it("should throw an error when downloading a non-existent file", async ({
    ...
    await expect(
      fileStorage.downloadFile("nonExistentFile.txt")
    ).rejects.toThrow(/File nonExistentFile.txt not found/);
  });
});
